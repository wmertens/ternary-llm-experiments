"""gpt_bop — trainer for a vanilla ternary GPT decoder.

Same ternary infrastructure as hrm_bop (random-init trits, per-(row, group)
scales, BopTernary/CMuon-STE on trits, Lion32/CMuon on FP/scales) but the
model is a flat stack of decoder layers — no recurrence, no H/L split, no
cycle sweep. Purpose: isolate ternary training speedups from the
recurrence confound.

Most of the optimizer / init / freezing / checkpoint plumbing is copied
verbatim from hrm_bop because it operates on QLinear modules, not the HRM
recurrence. The diff is concentrated in: GptBopConfig replaces HrmBopConfig
(no H_layers/L_layers/H_cycles/L_cycles); the train-loop forward is one
plain call to model(input_ids); the cycle-sweep eval and per-loop-CE
diagnostic are dropped; --grad-mode / --min-h-cycles / --max-h-cycles /
--eval-cycle-sweep / --full-bptt-steps are removed.
...
from .hrm_model import RMSNorm
from .gpt_model import GptBopConfig, GptBopModel
...
def freeze_non_embed_fp(model: GptBopModel) -> int:
    """Freeze every FP param except embed_tokens. RMSNorm weights and an
    untied lm_head get frozen; embed_tokens stays trainable. Used in the
    Bop-isolation experiment: only the linguistic prior moves under Lion,
    all other FP structure is fixed."""
    n = 0
    for m in model.modules():
...
def split_params(model: GptBopModel) -> tuple[list[nn.Parameter],
                                              list[nn.Parameter],
                                              list[nn.Parameter]]:
    ...
    fp_params: every other parameter — embeddings, RMSNorm weights,
        optional lm_head (managed by Lion).
    """
...
def trit_stats_per_stack(model: GptBopModel) -> dict[str, float]:
    """No-op for flat GPT stack — diag returned by flip_distill.trit_stats
    already covers the overall fraction-zero. Kept as a stub so the train
    loop can call it unconditionally."""
    return {}


def scale_stats_per_stack(model: GptBopModel) -> dict[str, float]:
    return {}
...
def save_safetensors(model: nn.Module, path: Path,
                     cfg: GptBopConfig, extra_meta: dict | None = None) -> None:
...
@torch.no_grad()
def evaluate(model: GptBopModel, val_batches: list[dict],
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--freeze-non-embed-fp", action="store_true", default=False,
                    help="Freeze RMSNorm weights, lm_head. Embed "
...
def main() -> None:
    ...
    if args.freeze_non_embed_fp:
        ...
        print(f"[freeze] non-embed FP frozen ({nf} tensors: norms + "
              f"lm_head if untied)", flush=True)
    ...
    writer.add_text("config", "  \n".join([
        "**gpt_bop** — vanilla ternary GPT decoder",
        f"- model: layers={cfg.num_layers}, hidden={cfg.hidden_size}, "
    ...
    writer.flush()
    writer.close()
    # Final on-disk save of the full training state so a follow-up run can
    # extend training from this exact step with optimiser momentum, LR
    # phase, and EMA tracker preserved. Without this the cosine schedule
    # and CMuon/Bop/Lion buffers would reset on resume, wasting effective
    # steps. Convert to safetensors-only via tools/dump_interrupted.py
    # if you need a deployment artifact.
    _save_resume(interrupted_path, model, opt_bop, opt_cmuon,
                 opt_lion_trits, opt_lion, global_step,
